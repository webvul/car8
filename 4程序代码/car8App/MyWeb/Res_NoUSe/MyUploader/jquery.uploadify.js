...
Help http://www.uploadify.com/documentation/
*/
	...
	function (jQuery) {
	    jQuery.extend(jQuery.fn, {
	        uploadify: function (options) {
	            jQuery(this).each(function () {
	                settings = jQuery.extend({
	                    ...
	                    buttonText: "",
	                    buttonImg: "",
	                    scriptData: false,   //传递到服务器的参数,是一个对象.
	                ...
	                data.buttonImg = settings.buttonImg;
	                ...
	                jQuery(this).bind("uploadifySelect", { 'action': settings.onSelect, 'queueID': settings.queueID }, function (event, ID, fileObj) {
	                    if (event.data.action(event, ID, fileObj) !== false) {
	                        ...
	                        jQuery(queue).offset({ left: $('#' + jQuery(this).attr('id') + "Uploader").offset().left });
	                        settings.onSelected();
	                    }
	                });
	            ...
	            // end uploadify 
	            $.timer(1000, function (tim) {
	                if (!$("#" + this.settings.id  + "Uploader")[0].clearFileUploadQueue) {
	                    alert("请检查浏览器是否有Flash插件。");
	                }
	                tim.stop();
	            });
	        },
	        ...
	        uploadifyUpload: function (ID) {
	            jQuery(this).each(function () {
	                $("#" + $(this).attr("id") + "Uploader")[0].startFileUpload(ID, false);
	            });

	        },
	        uploadifyCancel: function (ID) {
	            jQuery(this).each(function () {
	                $("#" + $(this).attr("id") + "Uploader")[0].cancelFileUpload(ID, true, false);
	            });
	        },
	        uploadifyClearQueue: function () {
	            jQuery(this).each(function () {
	                var swf = $("#" + $(this).attr("id") + "Uploader")[0];
	                if (swf.clearFileUploadQueue) {
	                    swf.clearFileUploadQueue(false);
	                }
	            });
	        }
