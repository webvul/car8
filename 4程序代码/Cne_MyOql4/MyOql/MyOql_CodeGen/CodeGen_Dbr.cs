...
【endfor】
    ...
    {
        if (MyOql_EntityDict.ContainsKey(DbName)) return MyOql_EntityDict[DbName] ;
            ...
            strEnt = strEnt
                ...
                .DoEach((SysEnts, _SysEntsString) =>
                {
                    ...
                    return _SysEntsString
                        ...
                        {
                            var funcParas = Paras as MyOqlCodeGenSect.TableCollection.TableGroupCollection.TableElement;
                            if (funcParas == null) return _PrarsString;
                            return _PrarsString.TmpFor("Paras", funcParas.Paras)
                                 .DoIf("NotFirst", o => funcParas.Paras.IndexOf(p => p.Name == o.Name) > 0)
                                 .DoFor("ParaType", o => GetTypeMap(dbo.GetDatabaseType(Paras.db)).First(t => t.DbType == o.DbType).CsType.FullName)
