    ...
    <script type="text/javascript">
        $(function () {
            ...
            jv.page().flexiView = function (id) {
                Boxy.load("~/Admin/AutoGen/EntityTableDetail/" + id + ".aspx", { filter: ".Main:last", modal: true, title: "查看详细" }, function (bxy) {
            ...
            jv.page().flexiAdd = function () {
                Boxy.ask(
                    jv.GenKv("tab", "表名", "") + jv.GenKv("config", "数据库配置", "dbo") + jv.GenKv("owner", "所有者", "dbo"), ["确定"], function (res, bxy) {
                        if (res == "确定") {
                            Boxy.load(
                            "~/Admin/AutoGen/EntityTableAdd/" + bxy.boxy.find("#tab").val() + ".aspx?DbConfig=" + bxy.boxy.find("#config").val() + "&Owner=" + bxy.boxy.find("#owner").val(),
            ...
            jv.page().flexiEdit = function (id) {
                Boxy.load("~/Admin/AutoGen/EntityTableUpdate/" + id + ".aspx", { filter: ".Main:last", modal: true, title: "修改" }, function (bxy) {
            ...
            jv.page().flexiDel = function (ids, grid) {
                //删除。
                $.post("~/Admin/AutoGen/EntityTableDelete" + ".aspx", { query: ids.join(",") }, function (res) {
            ...
            myGrid.flexigrid({
                ...
                //                    { display: "模块ID", name: "Id", width: "auto",  align: "left" },
                    ...
                    { display: "主键", name: "PKs", width: 50,  align: "left" },
                    { display: "自增列", name: "AutoIncreKey", width: 50,  align: "left" },
                    { display: "唯一列", name: "UniqueKey", width: 50,  align: "left" },
                    { display: "外键", name: "FKs", width: 280,  align: "left" },
                    { display: "指定列类型", name: "Enums", width: 280,  align: "left" },
                    { display: "所有者", name: "Owner", width: 50,  align: "left" },
                    { display: "数据库", name: "db", width: 50,  align: "left" },
            ...
            myGrid.getFlexi().p.url = "~/Admin/AutoGen/EntityTableQuery" + (jv.page()["uid"] ? "/" + jv.page()["uid"] : "") + ".aspx";
